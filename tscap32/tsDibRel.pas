...
function BitsOfPackedDib(pPackedDib: PBITMAPINFO): PAnsiChar;
...
function CopyDib(pOrigDibBmi: PBITMAPINFO; pOrigDibBits: PAnsiChar): PBITMAPINFO;

//copies and crops a Dib (works at the time only with 8bpp/24bpp and BI_RGB)
//returns a packed Dib
//if pOrigDibBits is NULL, the function assumes pOrigDibBmi as a Pointer to an packed Dib
//returns NULL if an Error occurred
function CopyAndCropDib(pOrigDibBmi: PBITMAPINFO; pOrigDibBits: PAnsiChar; x, y, w, h: Integer): PBITMAPINFO;
...
procedure SaveDib(pBmi: PBITMAPINFO; pBits: PAnsiChar; FileName: String);
...
//if pBits is NULL, the function assumes pBits as a Pointer to an packed Dib
function CreateTBitmapFromDib(pBmi: PBITMAPINFO; pBits: PAnsiChar):TBitmap;
...
function DecompressDib(pBmi: PBITMAPINFO; pBits: PAnsiChar): PBITMAPINFO;
...
function BitsOfPackedDib(pPackedDib: PBITMAPINFO): PAnsiChar;
begin
  Result := PAnsiChar(pPackedDib) + GetBmiSize(pPackedDib);
end;
